...
THIS TESTING VERSION LOGS ALL THE TIMES. THE REGULAR VERSION ONLY LOGS TIMES ABOVE 1395 SECONDS."""
text3 = """                                   Kristijonas Jankauskas 2022 June
                                              Deep Focus Logbook v1.0(TESTING VERSION)
    ...
    if running:
        ...
        if second <= 100:
            BarPercentage.config(
                text=f"{round((second / 100) * 100, 1)}% to deep focus!")
        if second > 100:
...
def BarProgressing():
    ...
    bar['value'] = second2 / 1
    if second <= 100:
        percent = f"{round((second / 100) * 100, 1)}%"
    elif second > 100:
        percent = "100%"
    else:
        pass
